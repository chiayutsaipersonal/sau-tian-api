...
router
  ...
  .get(
    '/:productId/conversionFactors/:conversionFactorId',
    (req, res, next) => {
      return productQueries
        .findDuplicates(req.params.productId, req.params.conversionFactorId)
        .then(data => {
          req.resJson = { data }
          next()
  ...
  .get('/', pagination(productQueries.recordCount), (req, res, next) => {
    let query = req.linkHeader
      ? productQueries.getProducts(
        req.queryOptions.limit,
        req.queryOptions.offset
      )
      : productQueries.getProducts()
    return query
      .then(data => {
  ...
  .post('/upload', multer.single('conversionFactors'), (req, res, next) => {
  ...
  .post(
    ...
    (req, res, next) => {
      ...
      return productQueries
        .batchSequentialUpdate(convertedData)
        .then(() => fs.remove(req.file.path))
        .then(() => {
          req.resJson = {
            message: 'Custom stock quantity information uploaded',
          }
          next()
          return Promise.resolve()
        })
